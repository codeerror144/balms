...
def load_registered_faces():
    ...
    for row in results:
        ...
        if img_path.startswith("public/"):
            absolute_path = os.path.join("C:/laragon/www/bits_logbook", img_path)  # Directly use the path from the database
        else:
            absolute_path = os.path.join("C:/laragon/www/bits_logbook/public", img_path)  # Add 'public/' only if missing
...
def save_unknown_user_face(frame, top, right, bottom, left):
    ...
    # Saving the image under 'public/unregister_user'
