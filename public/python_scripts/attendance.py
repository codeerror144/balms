...
# Cooldown dictionary for face recognition
last_recognized = {}
...
def load_registered_employee_faces():
    ...
    for row in results:
        ...
        # Check if 'public/' is already part of the path and adjust accordingly
        if img_path.startswith("public/"):
            absolute_path = os.path.join("C:/laragon/www/bits_logbook", img_path)  # Use the full path for the image file
        else:
            absolute_path = os.path.join("C:/laragon/www/bits_logbook/public", img_path)  # Add 'public/' if missing
